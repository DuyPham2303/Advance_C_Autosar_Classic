...
Status Build_SearchList()
{
    ...
    UserNode* pSortedName = Read_ListUser(SORT_NAME);
    UserNode* pSortedPhone = Read_ListUser(SORT_PHONE);

    //trả về kết quả xây dựng cây nhị phân cho tên và sdt
    RootByName = Build_SelectedTree(pSortedName);
    RootByPhone = Build_SelectedTree(pSortedPhone);
