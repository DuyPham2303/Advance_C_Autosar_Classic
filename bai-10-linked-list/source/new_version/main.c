...
void test(){
    ...
    if (code == 0)
    {
        ...
        if (st != LIST_OK) {
            ...
            int v;
            if (front(head, &v) == LIST_OK) printf("front = %d\n", v);
        }
...
int main(void)
{
        ...
        while (running)
        {
            ...
            case 1: {
                int v; printf("value: "); if (scanf("%d", &v) != 1) { printf("bad value\n"); break; }
                ListStatus st = push_front(&head, v);
                if (st != LIST_OK) {
                    list_log(st, "push_front failed");
                    if (st == LIST_ALLOC_FAILED) list_throw(st, "push_front allocation failed");
                }
                break; }
            ...
            case 2: {
                ...
                if (st != LIST_OK) {
                    list_log(st, "push_back failed");
                    if (st == LIST_ALLOC_FAILED) list_throw(st, "push_back allocation failed");
                }
                break; }

            case 3: {
                ListStatus st = pop_front(&head);
                if (st != LIST_OK) list_log(st, "pop_front failed");
                break;
            }

            case 4: {
                ListStatus st = pop_back(&head);
                if (st != LIST_OK) list_log(st, "pop_back failed");
                break;
            }

            case 5: {
                int pos, val; printf("pos value: "); if (scanf("%d %d", &pos, &val) != 2) { printf("bad input\n"); break; }
                ListStatus st = insert(&head, val, pos);
                if (st != LIST_OK) list_log(st, "insert failed");
                break;
            }

            case 6: {
                int pos; printf("pos: "); if (scanf("%d", &pos) != 1) { printf("bad input\n"); break; }
                ListStatus st = erase(&head, pos);
                if (st != LIST_OK) list_log(st, "erase failed");
                break;
            }

            case 7: {
                int out; ListStatus st = front(head, &out);
                if (st == LIST_OK) printf("front = %d\n", out); else list_log(st, "front failed");
                break;
            }

            case 8: {
                int out; ListStatus st = back(head, &out);
                if (st == LIST_OK) printf("back = %d\n", out); else list_log(st, "back failed");
                break;
            }

            case 9: {
                int pos; printf("pos: "); if (scanf("%d", &pos) != 1) { printf("bad input\n"); break; }
                int out; ListStatus st = get(head, pos, &out);
                if (st == LIST_OK) printf("get(%d) = %d\n", pos, out); else list_log(st, "get failed");
                break;
            }

            case 10: printf("size = %d\n", size(head)); break;

            case 11: printNode(head); break;

            case 12: { ListStatus st = clear(&head); if (st != LIST_OK) list_log(st, "clear failed"); break; }
