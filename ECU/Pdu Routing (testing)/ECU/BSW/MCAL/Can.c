...
void Can_MainFunction_Read(void) {
    //kiểm tra CAN đã được khởi tạo thành công mới xử lý tiếp
    if (!canInitialized) {
        return;
    }

    /* Gọi csv_lib để lấy chuỗi PDU (ví dụ "0x3F0 8 03 13 BD ...") */
    char *frameStr = getDataPdu("can"); //kiểm tra chuỗi nhập từ bàn phím
    if (!frameStr) {
        /* Không có frame mới, hoặc lỗi mở file */
        return;
    }

    //Khai báo biến lưu gói CAN
    PduInfoType rxPdu;
    memset(&rxPdu, 0, sizeof(rxPdu));

    /* 1) Lấy CAN ID */ 
    char *token = strtok(frameStr, " ");    //tách byte đầu lưu ID 
    if (!token) {
        free(frameStr);
        return;
    }
