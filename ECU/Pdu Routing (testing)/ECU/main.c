...
int main(void) {
    ...
    Can_MainFunction_Read();
    /* 2. Tạo thread cho CAN polling -> mô phỏng ngắt CAN, chạy độc lập với các luồng chạy các task (Swcs)*/
    ...
    while (1) {
        ...
        Rte_SpeedControl_MainFunction();    //mô phỏng chạy định kỳ các Swc thông qua Rte interface
        sleep(1);
